...
const currencyFormatter = function currencyFormatter(props) {
	const getFormatter = function getFormatter(options) {
		let locale = void 0
		let currency = void 0
		let symbol = void 0
		let pattern = void 0
		...
		// Helper Functions
		const isUndefined = function isUndefined(o) {
			return typeof o === 'undefined'
		}

		const toFixed = function toFixed(n, precision) {
		...
		currency = isUndefined(options.currency) || !options.currency ? 'USD' : options.currency.toUpperCase()
		...
		const encodePattern = function encodePattern(pattern) {
			let decimalPlaces = 0
			let frontPadding = ''
			let backPadding = ''
			const groupLengths = []
		...
		const pad = function pad(n, width) {
			n += ''
			return n.length >= width ? n : new Array(width - n.length + 1).join('0') + n
		}

		// Format function
		const format = function format(n, f) {
			...
			var formattedNumber =
				f.frontPadding + segment + (isUndefined(splitNumber[1]) ? '' : f.decimal + splitNumber[1]) + f.backPadding
		...
		const negativeFormat = isUndefined(patternArray[1])
	...
	const format = function format(number, options) {
...
function getCurrencySeparators(locale) {
	if (!locale) return

	let separatorInfo = locales[locale]
	if ('h' in separatorInfo) separatorInfo = locales[separatorInfo.h]

	return { decimalSeparator: separatorInfo.d, groupingSeparator: separatorInfo.g }
}

const getCurrencyInfo = function getCurrencyInfo(currency = 'USD') {
	const currencySymbol = symbols[currency] || '$'
	const localeIdentifier = defaultLocales[currency] || 'en_US'
	const separatorInfo = getCurrencySeparators(localeIdentifier)

	return { ...separatorInfo, currencySymbol, localeIdentifier }
}

export { currencyFormatter, getCurrencyInfo }

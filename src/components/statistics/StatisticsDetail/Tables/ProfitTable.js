...
class ProfitTable extends Component {
  ...
  renderThs() {
    const thead = [
      ...
      { th: I18n.t('statisticProfits'), status: 'bold', style: styles.alignEnd },
    ];
  ...
  renderTableRows() {
    ...
    const filterData = data.filter(s => s[dataInfo.unity]);
    ...
    return data.map((item, i) => {
      ...
      return (
        <KyteTableRow key={i}>
          ...
          <KyteTd status={status() || 'bold'} style={styles.alignEnd}><CurrencyText value={item.totalProfit} /></KyteTd>
        </KyteTableRow>
      );
    });
  }
...
export default connect(mapStateToProps, null)(ProfitTable);

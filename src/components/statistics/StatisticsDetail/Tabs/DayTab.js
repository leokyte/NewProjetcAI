...
class DayTab extends Component {
  constructor(props) {
    super(props);
    this.dayViewShotRef = React.createRef();
  }
  ...
  shareStatistics() {
    ...
    const viewShot = this.dayViewShotRef.current;
    if (!viewShot) {
      return;
    }
  ...
  renderLineCurvedChart(chartData) {
    const { sales } = this.props.statisticsData;
    const { periodRange, currencySymbol, dataInfo } = this.props;
    ...
    const listIndexData = listIndex(sales.length);

    const keys = () => sales.map((item) => {
      if (periodDuration > 2) return moment(item._id).format('MMM');
      return moment(item._id).format(dateFormat);
    });
  ...
  render() {
    ...
    const { sales } = this.props.statisticsData;
    const { dataInfo, periodRange } = this.props;
    const chartData = (arr = []) => arr.map((item) => { return item[dataInfo.unity]; });
    const startDate = new moment(periodRange.startDate);
    const endDate = new moment(periodRange.endDate);

    const dateFormat = (startDate.month() === endDate.month()) ? 'D' : 'D/MMM';

    const renderTable = () => {
      if (dataInfo.type === 'profit') {
        return <ProfitTable data={sales} dateFormat={dateFormat} />;
      } else if (dataInfo.type === 'taxes') {
        return <TaxesTable data={sales} dateFormat={dateFormat} />;
      }
      return <SalesTable data={sales} dateFormat={dateFormat} />;
    };

    return (
      <ScrollView style={outerContainer}>
        <ViewShot style={{ flex: 1 }} ref={this.dayViewShotRef} options={{ format: 'png', quality: 0.9, result: 'data-uri' }}>
          {this.renderSubTitle()}
          <LinearGradient colors={['white', colors.lightBg]} style={statisticGraphic}>
            {this.renderLineCurvedChart(chartData(sales))}
          </LinearGradient>
          {renderTable()}
        </ViewShot>
      </ScrollView>

...
class YearTab extends Component {
  constructor(props) {
    super(props);
    this.yearViewShotRef = React.createRef();
  }
  ...
  shareStatistics() {
    ...
    const viewShot = this.yearViewShotRef.current;
    if (!viewShot) {
      return;
    }
    viewShot.capture().then(uri => {
      Share.open(selectOs(uri)).then(() => {
        logEvent(`StatisticsDetail${title}Shared`, null);
      });
    });
  }
  ...
  render() {
    ...
    return (
      <ScrollView style={outerContainer}>
        <ViewShot style={{ flex: 1 }} ref={this.yearViewShotRef} options={{ format: 'png', quality: 0.9, result: 'data-uri' }}>
          {this.renderSubTitle()}
...
const styles = {
  ...
  chartInset: { top: 20, bottom: 0, right: 10, left: 10 },
  containerBg: {
    backgroundColor: '#FFF',
  },
...
export default connect(mapStateToProps)(YearTab);
